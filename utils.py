...
def filter(P_bar_user, P_bar_group, group_alpha, filter_pair):
    ...
    if filter_pair == 'filter_1D_1D':
        ...
        new_P = P_bar_user + group_alpha *(P_bar_group + beta*((-1)*P_bar_group@P_bar_group@P_bar_group + 10*P_bar_group@P_bar_group+ (-29)*P_bar_group))
        return new_P
        ...
        new_P = 2*P_bar_user - P_bar_user@P_bar_user + group_alpha*(P_bar_group + beta*((-1)*P_bar_group@P_bar_group@P_bar_group + 10*P_bar_group@P_bar_group + (-29)*P_bar_group))
        return new_P

    elif filter_pair == 'filter_3D_1D':
        new_P = P_bar_user + beta*((-1)*P_bar_user@P_bar_user@P_bar_user + 10*P_bar_user@P_bar_user + (-29)*P_bar_user) + group_alpha*P_bar_group
        return new_P

    elif filter_pair == 'filter_3D_2D':
        new_P = P_bar_user + beta*((-1)*P_bar_user@P_bar_user@P_bar_user + 10*P_bar_user@P_bar_user + (-29)*P_bar_user) + group_alpha*(2*P_bar_group - P_bar_group@P_bar_group)
        return new_P

    elif filter_pair == 'filter_3D_3D':
        new_P = P_bar_user + beta*((-1)*P_bar_user@P_bar_user@P_bar_user + 10*P_bar_user@P_bar_user + (-29)*P_bar_user) + group_alpha*(P_bar_group + beta*((-1)*P_bar_group@P_bar_group@P_bar_group + 10*P_bar_group@P_bar_group + (-29)*P_bar_group))
        return new_P
    
def filter_ablation_no_user(P_bar_user, P_bar_group, group_alpha, filter_pair):
    beta = 0.01
    if filter_pair == 'filter_1D_1D':
        new_P =  group_alpha * P_bar_group
        return new_P

    elif filter_pair == 'filter_1D_2D':
        new_P = group_alpha*(2*P_bar_group - P_bar_group@P_bar_group) 
        return new_P

    elif filter_pair == 'filter_1D_3D':
        new_P = group_alpha *(P_bar_group + beta*((-1)*P_bar_group@P_bar_group@P_bar_group + 10*P_bar_group@P_bar_group+ (-29)*P_bar_group))
        return new_P
...
def filter_ablation_no_group(P_bar_user, P_bar_group, group_alpha, filter_pair):
    beta = 0.01
    if filter_pair == 'filter_1D_1D':
        new_P =  P_bar_user 
        return new_P
        ...
        new_P = 2*P_bar_user - P_bar_user@P_bar_user 
        return new_P
        ...
        new_P = P_bar_user + beta*((-1)*P_bar_user@P_bar_user@P_bar_user + 10*P_bar_user@P_bar_user + (-29)*P_bar_user)
        return new_P
...
def hit_at_k(gt_mat, results, negative_list, k=10):
    hit_sum = 0 # hit_sum: 유저별로 hit한 아이템의 개수
    for i in range(gt_mat.shape[0]): # gt_mat.shape[0]: 유저의 수
        relevant_items = set(np.where(gt_mat[i, :] > 0)[0]) # 유저 i가 평가한 아이템의 인덱스
        candidates = list(set(relevant_items).union(set(negative_list[i]))) # 유저 i가 평가한 아이템과 유저 i가 평가하지 않은 아이템의 인덱스
        candidates = [item for item in candidates if item < results.shape[1]]  # 유효한 인덱스만 남김
        top_predicted_items = np.argsort(-results[i, candidates])[:k] # 유저 i에 대한 예측된 아이템 중 상위 k개
        top_predicted_items = [candidates[item] for item in top_predicted_items] # 예측된 아이템의 인덱스
        if len(relevant_items.intersection(top_predicted_items)) > 0: # 유저 i가 평가한 아이템 중 예측된 상위 k개의 아이템이 있으면
            hit_sum += 1
    hit_rate = hit_sum / gt_mat.shape[0] # hit_rate: hit한 유저의 비율
    return hit_rate
